...
import { getDashboardDirectoryPath } from "@/lib/config";
import type {
  DashboardDefinition,
  DashboardFile,
  DashboardFilter,
...
export async function ensureDashboardDirectory(): Promise<string> {
  const directoryPath = getDashboardDirectoryPath();
  await fs.mkdir(directoryPath, { recursive: true });
  return directoryPath;
}

export async function listDashboardFiles(): Promise<DashboardFile[]> {
...
export async function getDashboardFile(slug: string): Promise<DashboardFile | null> {
...
export async function deleteDashboardFile(slug: string): Promise<boolean> {
  const filePath = path.join(await ensureDashboardDirectory(), `${slug}.yaml`);
