...
import type { DashboardFilter, FieldMetadata, FilterValue, NoteRecord, SupportedFieldType } from "@backend/domain/models";
...
function buildRow(note: NoteRecord, columns: string[], obsidianUrl: string): QueryResultRow {
  const fieldValues = Object.fromEntries(columns.map((column) => [column, note.fields[column]?.value]));
